...
// First remove the bad placement
const badBlockStart = content.indexOf('      {/* Modal for Moving Dialogue */}');
const badBlockEnd = content.indexOf('function getVocabForConversation');

if (badBlockStart !== -1 && badBlockEnd !== -1) {
  content = content.substring(0, badBlockStart) + content.substring(badBlockEnd);
}

// Now insert it correctly just before the last closing `</div>` of ConversationDetail
const correctInsertionPoint = '      {!isAdding && viewMode === "list" && (';
...
content = content.replace(correctInsertionPoint, modalHTML + '\n' + correctInsertionPoint);
