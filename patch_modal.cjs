...
// We must remove the inline movingDialogueId render
const inlinePattern = /\{movingDialogueId === dialogue\.id && \([\s\S]*?\}\)[\s\S]*?\}\)[\s\S]*?<\/div>[\s\S]*?\}\)[\s\S]*?<\/div>[\s\S]*?\}\)/;
// Wait, the regex could be dangerous. Let's just do an exact replacement using split.
