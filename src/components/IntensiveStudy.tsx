...
import { IntensiveWord, IntensiveExample, WordCategory } from '../types';
...
export default function IntensiveStudy({ deck, onAddWord, onRemoveWord, onUpdateWord }: IntensiveStudyProps) {
  ...
  const renderExampleHighlight = (example: string, targetKanji: string) => {
    if (!example || !targetKanji) return <Fragment>“{example}”</Fragment>;
    
    // Exact match kanji/word
    if (example.includes(targetKanji)) {
    ...
    // Try kanji stem
    const kanjiChars = targetKanji.match(/[\u4e00-\u9faf]+/g);
        ...
        {isEditing ? (
          ...
          <div className="relative z-10 flex flex-col sm:flex-row gap-6 items-center sm:items-stretch h-full">
            ...
            <div className="flex-1 flex flex-col justify-center text-center sm:text-left h-full pr-8">
              ...
              {word.explanation && (
                <p className="text-[#d4d4d4]/90 text-sm sm:text-base leading-relaxed bg-[#1a1a1a]/50 p-4 rounded border-l-2 border-l-[#c5a059] mt-2">
                  {word.explanation}
                </p>
              )}
            </div>
        ...
        {word.examples.map((ex, index) => (
          ...
          >
             <button
                ...
                className="absolute top-2 right-2 p-2 text-[#d4d4d4]/20 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all rounded hover:bg-[#121212] z-10"
                title="Xoá ví dụ"
              >
                <Trash2 className="w-4 h-4" />
              </button>
