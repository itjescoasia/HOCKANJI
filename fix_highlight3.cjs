...
const startIdx = content.indexOf('export const RelatedHighlight: React.FC<{');
const endIdx = content.indexOf('import { KanjiCard } from "../types";'); // Wait, the imports might be single quotes

let endTarget = "import { KanjiCard } from '../types';";
let actualEndIdx = content.indexOf(endTarget);

if (actualEndIdx === -1) {
    actualEndIdx = content.indexOf("import { KanjiCard } from");
}
...
`;
    ...
    console.log("Could not find start or end index", startIdx, actualEndIdx);
}
